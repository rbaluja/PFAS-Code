#read in birth data
path = "/Users/nhdata/Library/CloudStorage/Box-Box/[UA Box Health] Economics/[UA Box Health] VR2210_Deliverable/dr_6264_deliverable.xlsx"
df = read_excel(path, sheet = 2)
rm(path)

#drop individuals with missing lat long for now (5390 = 5318 with address info & 72 NAs)
df = df %>%
  dplyr::filter(!is.na(POINT_X) & POINT_X != 0) %>% 
  dplyr::rename(lng = POINT_X, lat = POINT_Y)

#clean up remainder of variables
df = df %>% 
  dplyr::filter(CHLD_SEX != "U") %>% #remove individuals with unknown sex (6)
  dplyr::mutate(male = ifelse(CHLD_SEX == "M", 1, 0)) %>% 
  ...
  dplyr::filter(!is.na(CHLD_BRTH_WGT)) %>% #drop ind with missing birthweight info
...
#drop individuals with 9999 birthweight (198) or 99 gestation (469)
df = df %>% 
  dplyr::filter(bweight != 9999 & gestation != 99)

# #bring in county (merge by zipcode using HUD's HUD-USPS Crosswalk)
# zc_cross = read_excel("Data/Supplemental/ZIP_COUNTY_122021.xlsx") %>% 
#   dplyr::filter(usps_zip_pref_state == "NH") %>% 
#   dplyr::group_by(zip) %>% 
#   dplyr::filter(res_ratio == max(res_ratio)) %>% #for zipcodes that span multiple counties, take the one which has the max residential ratio
#   ungroup() %>% 
#   dplyr::rename(city = usps_zip_pref_city, 
#                 zipcode = zip) %>%
#   dplyr::select(zipcode, county, city) 
# 
# df = df %>% 
#   left_join(zc_cross)
