...
figure_fun = function(reg, var, radius, n_triangles, sections, color = "Reds", title = "none"){
  ...
  #replace tr with close when km = 2
  fdf$tr = as.character(fdf$tr)
  fdf[fdf$lev == 2, ]$tr = "close"
  
  #take union of close geometries
  fdf_close = fdf %>% 
    dplyr::filter(tr == "close")
  
  fdf_close$geometry = fdf_close %>% st_union(.)
  fdf_close = fdf_close %>% unique()
  
  #return back to fdf
  fdf = fdf %>% 
    dplyr::filter(tr != "close") %>% rbind(fdf_close)
  ...
  coef_df$tr = ifelse(coef_df$tr == "tclose", "close", parse_number(coef_df$tr))
  ...
  fdf$al = ifelse(fdf$stars == 3, 0.95, 
  ...
  p = ggplot() + geom_sf(data = fdf, aes(fill = coef, alpha = al)) +
    ...
    #   size = 2) +
    scale_fill_distiller(palette = color, direction = -1) +theme_void() + 
