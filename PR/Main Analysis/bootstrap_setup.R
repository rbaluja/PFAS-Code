...
fs_cont[is.na(fs_cont$n_sites_down), ]$n_sites_down = 0
fs_cont[is.na(fs_cont$n_sites_up), ]$n_sites_up = 0
...
fs_cont = dplyr::bind_rows(pblapply(1:nrow(fs_cont), fs_cont_dist))
...
fs_cont_assgn = function(i, drop_far_down, drop_far_up){
  ...
  if (!is.na(d_up) & down_far == 0){ #if there is an up site (and no down sites)
    if (d_up < meters){ #if the up site is within the buffer, assign its values to the well
      w$pfas = w$pfas_up
      w$site = w$site_up
      w$dist = w$dist_up
      w$up = 1
...
if (soil_well == TRUE){
  ...
  fs_cont = fs_cont %>% 
    left_join(cont_sites %>% dplyr::select(sp, awc, site)) 
}
...
if (soil_well == TRUE){
  ...
  df = df %>% left_join(cont_sites %>% as_tibble() %>% dplyr::select(site, awc, sp)) 
}
