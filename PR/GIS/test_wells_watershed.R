...
test_watershed = function(i){
  ...
  wbt_snap_pour_points(pour_pts = temp_point_path, 
                       ...
                       output = paste0("New Hampshire/Data/QGIS/fs_test_pp/pp_site_", i, ".shp"), 
                       snap_dist = 0.007569 * 5)
  #calculate watershed
  wbt_watershed(d8_pntr = "New Hampshire/Data/QGIS/flow_dir.tiff", 
                pour_pts = paste0("New Hampshire/Data/QGIS/fs_test_pp/pp_site_", i, ".shp"), 
                output = paste0("New Hampshire/Data/QGIS/fs_test_watershed/watershed_", i, ".tiff"))
  #read in watershed
  ws = terra::rast(paste0("New Hampshire/Data/QGIS/fs_test_watershed/watershed_", i, ".tiff"))
  
  #transform watershed to a polygon
  ws_poly = as.polygons(ws)
  #save shapefile of watershed
  writeVector(ws_poly, paste0("New Hampshire/Data/QGIS/fs_test_watershed/Shapes/ws_shape_", i, ".shp"), overwrite = TRUE)
...
files = list.files("New Hampshire/Data/QGIS/fs_test_watershed/Shapes", pattern = "*.shp", recursive = T, full.names = T)
...
save(test_ws, file = "New Hampshire/Data/RData/fs_test_watershed.RData")
