figure2_still_fun_pws = function(data, category, keep_x, header, ti, left){
  ...
  if (header){
    ...
    res_plot$Check = factor(res_plot$Check, c("Baseline", "PWS Fixed Effects", "Model"
    ))
    res_plot$model = res_plot$Check
  }else{
    res_plot$Check = factor(res_plot$Check, c("Baseline", "PWS Fixed Effects",
    ))
    res_plot$model = res_plot$Check
  }
...
figure2_still_fun_imo = function(data, category, keep_x, header, ti, left){
  if (category == "Any" | category == "Any Low-Birthweight (Full-Term)"){
    col = "dodgerblue"
  }else if (category == "Slightly" | category == "Slightly"){
    col = "coral"
  ...
  if (header){
    ...
    res_plot$Check = factor(res_plot$Check, c("Baseline", "Include Missing Obs", "Model"
    ))
    res_plot$model = res_plot$Check
  }else{
    res_plot$Check = factor(res_plot$Check, c("Baseline", "Include Missing Obs",
    ))
    res_plot$model = res_plot$Check
  }
