...
boot_err_linear = function(i, df, fs_cont){
  ...
  w_reg = fixest::feols(wellpfas ~ down * poly(sp, awc, sand, clay, silt, degree = 1, raw = TRUE) + pfas + log(dist)*down + 
...
boot_err_nod = function(i, df, fs_cont){
  boot_coefs = data.frame(matrix(ncol = 11, nrow = 1))
  colnames(boot_coefs) = c("preterm", "lpreterm", "mpreterm", "vpreterm", 
                           "lbw", "llbw", "mlbw", "vlbw",
                           "gestation", "bweight", "mort")
  ...
  w_reg = fixest::feols(asinh(wellpfas) ~ down * poly(sp, awc, clay, sand, silt, degree = 1, raw = TRUE) + asinh(pfas) + log(dist)*down + 
                          ...
                          p_manuf + n_hunits + med_hprice + elevation + tri5 + t, data = fs_cont_bs %>% filter(domestic == 0)) 
...
boot_coefs = dplyr::bind_rows(pblapply(1:bts, boot_err_nod, df, fs_cont, cl = n_cores))
save(boot_coefs, file = modify_path("Data_Verify/RData/Revision2/bootstrap_ndomestic.RData")) 
