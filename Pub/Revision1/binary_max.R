...
down_well_dist = function(w){
  ...
  ns5 = length(which(ds <= meters))
  dw$dist_down = as.vector(ds)
  
  if (ns5 > 0){
    #subset to only downgradient sites within meters of the well
    dw = dw[which(dw$dist_down <= meters), ]
    #subset to the close site with the maximum cont. level
    dw = dw[which(dw$pfas == max(dw$pfas)), ]
    #for ties, select the first one
    dw = dw[1, ]
  }else{
...
up_well_dist = function(w){
  ...
  ns5 = length(which(ds <= meters))
  uw$dist_up = as.vector(ds)
  
  if (ns5 > 0){
    #subset to only upgradient sites within meters of the well
    uw = uw[which(uw$dist_up <= meters), ]
    #subset to the close site with the maximum cont. level
    uw = uw[which(uw$pfas == max(uw$pfas)), ]
    #for ties, select the first one
    uw = uw[1, ]
  }else{
    #get the index of the nearest relevant release site
    ind_nearest = which.min(ds)
    #get the site name for the nearest relevant release site
    rsuw_site = rs_ll[which(rs_ll$site %in% uw$site), "site"]
    nearest_site = as.character(rsuw_site[ind_nearest]) 
    #subset uw to only the nearest up site
...
#Find release site within meters with greatest cont for each well
well_dist = function(i){
  w = wells[i, ]
  
  dists = distm(c(w$lng, w$lat), rs_ll[, c("lng", "lat")])
  
  ns5 = length(which(dists <= meters))
  
  if (ns5 > 0){
    #subset to only upgradient sites within meters of the well
    s = rs_ll
    s$dist = as.vector(dists)
    s = s[which(s$dist <= meters), ]
    
    w$dist_near = s[which(s$pfas == max(s$pfas)), ]$dist[1]
    w$pfas_near = s[which(s$pfas == max(s$pfas)), ]$pfas[1]
    w$n_sites_meters = nrow(s)
    w$site_near = s[which(s$pfas == max(s$pfas)), ]$site[1]
  }else{
    #get the index of the nearest relevant release site
    ind_nearest = which.min(dists)
    
    w$dist_near = as.vector(dists)[ind_nearest][1]
    w$pfas_near = rs_ll$pfas[ind_nearest][1]
    w$n_sites_meters = 0
    w$site_near = rs_ll$site[ind_nearest][1]
  }
