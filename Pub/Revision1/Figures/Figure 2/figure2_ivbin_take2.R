...
creg = fixest::feols(pred_pfas ~ down + updown|county, data = df_est)
...
pfas_diff = creg$coefficients["down"] - creg$coefficients["updown"]

pre_ivc = (pfas_diff * preterm_iv)/mean(df_est$gestation < 37) * 100
pre_ivc_se = pfas_diff * preterm_sd/mean(df_est$gestation < 37) * 100

mpre_ivc = (pfas_diff * lpreterm_iv)/mean(df_est$gestation < 37 & df_est$gestation >= 32) * 100
mpre_ivc_se = pfas_diff * lpreterm_sd/mean(df_est$gestation < 37 & df_est$gestation >= 32) * 100

vpre_ivc = (pfas_diff * mpreterm_iv)/mean(df_est$gestation < 32 & df_est$gestation >= 28) * 100
vpre_ivc_se = pfas_diff * mpreterm_sd/mean(df_est$gestation < 32 & df_est$gestation >= 28) * 100

epre_ivc = (pfas_diff * vpreterm_iv)/mean(df_est$gestation < 28) * 100
epre_ivc_se = pfas_diff * vpreterm_sd/mean(df_est$gestation < 28) * 100

lbw_ivc = (pfas_diff * lbw_iv)/mean(df_est$bweight < 2500) * 100
lbw_ivc_se = pfas_diff * lbw_sd/mean(df_est$bweight < 2500) * 100

mlbw_ivc = (pfas_diff * llbw_iv)/mean(df_est$bweight < 2500 & df_est$bweight >= 1500) * 100
mlbw_ivc_se = pfas_diff * llbw_sd/mean(df_est$bweight < 2500 & df_est$bweight >= 1500) * 100

vlbw_ivc = (pfas_diff * mlbw_iv)/mean(df_est$bweight < 1500 & df_est$bweight >= 1000) * 100
vlbw_ivc_se = pfas_diff * mlbw_sd/mean(df_est$bweight < 1500 & df_est$bweight >= 1000) * 100

elbw_ivc = (pfas_diff * vlbw_iv)/mean(df_est$bweight < 1000) * 100
elbw_ivc_se = pfas_diff * vlbw_sd/mean(df_est$bweight < 1000) * 100

mort_ivc = (pfas_diff * mort_iv)/mean(df_est$death) * 100
mort_ivc_se = pfas_diff * mort_sd/mean(df_est$death) * 100
...
ggsave(paste0("Figures Revision/figure2_ivbin_take2", ppt, ".png"), fig2, width = 17000, height = 5500, units = "px", limitsize = F)
...
ggsave(paste0("Figures Revision/figure2_mort_ivbin_take2", ppt, ".png"), mort_f2, width = 11000, height = 2000, units = "px", limitsize = F)
