figure2_fun_np = function(data, category, keep_x, header, ti, left){
  ...
  if (header){
    ...
    res_plot$Check = factor(res_plot$Check, c( "Baseline", "W/O Paternal Education", "Model"
    ))
    res_plot$model = res_plot$Check
  }else{
    res_plot$Check = factor(res_plot$Check, c( "Baseline", "W/O Paternal Education"
    ))
    res_plot$model = res_plot$Check
  }
...
figure2_fun_npease = function(data, category, keep_x, header, ti, left){
  data <- data %>% arrange(match(Check, levels(Check)))
  if (category == "Any" | category == "Any Low Birthweight (Full-Term)"){
    col = "dodgerblue"
  }else if (category == "Moderately" | category == "Moderately"){
    col = "coral"
  ...
  if (header){
    ...
    res_plot$Check = factor(res_plot$Check, c("Baseline", "Drop Pease AFB Births", "Model"
    ))
    res_plot$model = res_plot$Check
  }else{
    res_plot$Check = factor(res_plot$Check, c("Baseline", "Drop Pease AFB Births"
    ))
    res_plot$model = res_plot$Check
  }
...
figure2_fun_cws = function(data, category, keep_x, header, ti, left){
  data <- data %>% arrange(match(Check, levels(Check)))
  if (category == "Any" | category == "Any Low Birthweight (Full-Term)"){
    col = "dodgerblue"
  }else if (category == "Moderately" | category == "Moderately"){
...
figure2_fun_ivbin = function(data, category, keep_x, header, ti, left){
  if (category == "Any" | category == "Any Low Birthweight (Full-Term)"){
    col = "dodgerblue"
  }else if (category == "Moderately" | category == "Moderately"){
    col = "coral"
  ...
  if (keep_x){
    pany1 = data %>% 
      arrange(match(Check, c("Model 1", "Model 2"))) %>%
      ggplot(aes(y = Check)) + 
      geom_point(aes(x=Estimate), color = col, shape=16, size=18) +
      geom_linerange(aes(xmin=d_lower, xmax=d_upper), size = 3) + 
      geom_vline(xintercept = 0, linetype="dashed") +
  ...
  if (header){
    ...
    res_plot$Check = factor(res_plot$Check, levels = c("Model 1", "Model 2", "Model"))
    res_plot$model = res_plot$Check
  }else{
    res_plot$Check = res_plot$Check = factor(res_plot$Check, c("Model 1", "Model 2"))
    res_plot$model = res_plot$Check
  }
...
figure2_fun_ivthresh = function(data, category, keep_x, header, ti, left){
  if (category == "Any" | category == "Any Low Birthweight (Full-Term)"){
    col = "dodgerblue"
  }else if (category == "Moderately" | category == "Moderately"){
    col = "coral"
  ...
  if (keep_x){
    pany1 = data %>% 
      ...
      theme(axis.line.y = element_blank(),
            ...
            axis.text.x = element_text(size = 46), 
            axis.title.x = element_text(size = 50)) + 
      guides(color = "none") + xlim(c(-0.05, 0.05)) + xlab("Change, in rate of incidence")
  }else{
    pany1 = data %>% 
      arrange(match(Check, c("Model 1", "Model 2"))) %>%
      ggplot(aes(y = Check)) + 
      ...
      theme(axis.line = element_blank(),
            ...
            axis.text= element_blank(),
            axis.title= element_blank()) + 
      guides(color = "none") + xlim(c(-0.05, 0.05))
  }
  ...
  if (header){
    ...
    res_plot$Check = factor(res_plot$Check, levels = c("500 ppt", "1000 ppt", "Threshold"))
    res_plot$model = res_plot$Check
  }else{
    res_plot$Check = res_plot$Check = factor(res_plot$Check, c("500 ppt", "1000 ppt"))
    res_plot$model = res_plot$Check
  }
  ...
  if (!header){
    ...
    p_left = 
      p_left +
      geom_text(aes(x = 0, label = Check), hjust = 0, 
                fontface = ifelse(res_plot$model == "Threshold", "bold", "plain"), size = 20)
  }
